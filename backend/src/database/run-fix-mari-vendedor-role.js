...
async function fixMariVendedorRole() {
  ...
  try {
    ...
    const sqlPath = path.join(__dirname, 'fix-mari-vendedor-role.sql');
    const sqlContent = fs.readFileSync(sqlPath, 'utf8');

    console.log('Ejecutando script para corregir el rol de Vendedor y asignarlo a Mari...');
    
    // Ejecutar el script SQL completo
    await client.query(sqlContent);
    
    // Verificar que Mari tenga el rol de Vendedor
