...
const AdditionalLoanForm: React.FC = () => {
  ...
  const [formData, setFormData] = useState<AdditionalLoanFormData>({
    ...
    const fetchData = async () => {
      if (isEditMode && id) {
        try {
          ...
          console.error('[ADDITIONAL_LOAN_FORM] Error loading data:', error);
          setErrorSnackbar({
            open: true,
            message: 'Error loading additional loan data'
          });
        } finally {
          setLoading(false);
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      if (isEditMode && id) {
        await updateAdditionalLoan(parseInt(id, 10), formData);
      } else {
        await createAdditionalLoan(formData);
      }
      
      navigate('/adicionales-prestamos');
    } catch (error) {
      console.error('[ADDITIONAL_LOAN_FORM] Error saving:', error);
      setErrorSnackbar({
        open: true,
        message: `Error ${isEditMode ? 'updating' : 'creating'} additional loan`
      });
    } finally {
      setLoading(false);
    }
...
export default AdditionalLoanForm;
